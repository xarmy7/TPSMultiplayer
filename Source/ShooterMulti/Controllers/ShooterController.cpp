...
void AShooterController::MoveForward(float Value)
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead() && ShooterCharacter->GetState() != EShooterCharacterState::PushButton)
	{
		...
		ShooterCharacter->AddMovementInput(Value * 100000 * ShooterCharacter->movementMultiplyer * Rotation.GetNormalized().Vector());
	}
...
void AShooterController::MoveRight(float Value)
{
		...
		&& ShooterCharacter->GetState() != EShooterCharacterState::PushButton)
	{
		...
		ShooterCharacter->AddMovementInput(Value * 100000 * ShooterCharacter->movementMultiplyer * Rotation.GetNormalized().RotateVector(FVector::RightVector));
	}
}
...
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead())
		ShooterCharacter->StartAim(true);
}

void AShooterController::EndAim()
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead())
		ShooterCharacter->EndAim(true);
}
...
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead())
		ShooterCharacter->EndReloadOnServer();
}

void AShooterController::PushButton()
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead())
		ShooterCharacter->PushButtonOnServer();
}

void AShooterController::Punch()
{
	if (IsValid(ShooterCharacter) && !ShooterCharacter->IsDead())
		ShooterCharacter->PunchOnServer();
}
